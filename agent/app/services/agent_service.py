...
class AgentService:
    ...
    async def _register(self):
        """Registra o actualiza el agente con el servidor."""
        ...
        registration_data = {
            ...
            "device_type": "Laptop" if "laptop" in platform.node().lower() else "PC",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{settings.SERVER_URL}/api/v1/agents/register", json=registration_data) as response:
                    ...
                    if response.status == 200:
                        print(f"✅ Agente registrado/actualizado con éxito en el servidor.")
                    else:
                        print(f"❌ Error en el registro: {data}")
        except Exception as e:
