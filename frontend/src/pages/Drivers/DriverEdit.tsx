...
const DriverEdit = () => {
  ...
  useEffect(() => {
    const fetchDriver = async () => {
      try {
        ...
        setManufacturer(driverData.manufacturer);
        setModel(driverData.model);
        setDescription(driverData.description || '');
        setError(null);
      } catch (err: any) {
        console.error('Error fetching driver:', err);
        setError(`Error al cargar el driver: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axiosInstance.put(`/drivers/${id}`, formData, {
        headers: {
      ...
      setError(`Error al actualizar el driver: ${err.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Editar Driver</h1>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="driver_file">
              Archivo del Driver {driver && <span className="font-normal text-gray-500">(Actual: {driver.driver_filename})</span>}
            </label>
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-gray-600">
                  ...
                  >
                    <span>Seleccione un nuevo archivo</span>
                    <input
                      id="driver_file"
                      name="driver_file"
                      type="file"
